...
function bkOnCarrierChange(){
  ...
  if(!carrier){
    ...
    if(btn) carrier = btn.getAttribute('data-value') || '';
  }
  bkPreviewData = null;
  bkSelectedDqns = [];
  bkAllMode = true;
  ...
  if(searchWrap){
    searchWrap.style.display = matches.length>0?'block':'none';
    searchWrap.style.opacity='0.4'; searchWrap.style.pointerEvents='none';
  }
...
function bkCollectData(){
  return {
    carrier: bkSelectedCarrier || (document.getElementById('bk_carrier')?document.getElementById('bk_carrier').value:''),
...
function bkDateAz(d){ return String(d.getDate()).padStart(2, '0') + '.' + String(d.getMonth() + 1).padStart(2, '0') + '.' + d.getFullYear(); }function bkToggleAllMode(){
  bkAllMode = !bkAllMode;
  var toggle=document.getElementById('bkAllToggle');
  var badge=document.getElementById('bkCountBadge');
  ...
  if(bkAllMode){
    ...
    if(srch){ srch.style.opacity='0.4'; srch.style.pointerEvents='none'; }
    bkSelectedDqns=[]; bkRenderDqnChips();
  } else {
    if(toggle) toggle.classList.add('off');
    if(badge) badge.classList.remove('active');
    if(srch){ srch.style.opacity='1'; srch.style.pointerEvents='auto'; }
  }
  bkUpdateDqnNotice();
  bkUpdateImportCount();
}
...
function bkToggleCarrierDD(){
  ...
  if(dd.style.display==='none'||dd.style.display===''){
    ...
    if(carriers.length===0){ ensureBulkFormData(); setTimeout(bkToggleCarrierDD,800); return; }
    dd.innerHTML=carriers.map(function(c){
      return '<div class="bk-carrier-dd-item'+(c===bkSelectedCarrier?' selected':'')+'" onclick="bkSelectCarrier(\'' + c.replace(/'/g,"\\'") + '\')">' + escapeHtml(c) + '</div>';
    }).join('');
...
function bkSelectCarrier(carrier){
  ...
  // hidden select sinxronizasiya
  var sel=document.getElementById('bk_carrier');
  if(sel){
    for(var i=0;i<sel.options.length;i++){
      if(sel.options[i].value===carrier){ sel.selectedIndex=i; break; }
    }
    if(sel.value!==carrier){
      var opt=document.createElement('option');
      opt.value=carrier; opt.textContent=carrier;
      sel.appendChild(opt); sel.value=carrier;
    }
  }
  bkOnCarrierChange();
}
...
document.addEventListener('click',function(e){
  if(!e.target.closest('.bk-carrier-select-wrap')){
    var dd=document.getElementById('bkCarrierDDList');
    if(dd) dd.style.display='none';
  }
  if(!e.target.closest('#bkDqnSearchCompact')){
    var sugg=document.getElementById('bkDqnSuggestions');
    if(sugg) sugg.classList.remove('open');
  }
});

// ── BK DQN Input - tire avtomatik ──
function bkDqnInputHandler(el){
  ...
  if(nums2)   formatted+='-'+nums2;
  el.value=raw.length>0?formatted:'';
  ...
  if(reg.length===0){
    ...
    reg.slice(0,10).forEach(function(r){
      ...
      (function(match){ div.addEventListener('click',function(){
        bkSelectDqn(match); el.value='';
        if(clr) clr.style.display='none';
        sugg.classList.remove('open'); el.focus();
      }); })(r);
...
// ── BK Collect Data - carrier-ı düzgün oxu ──
